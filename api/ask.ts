     ...
     export default async function handler(req: VercelRequest, res: VercelResponse) {
       ...
       try {
         ...
         try {
           ...
           res.status(200).json({ response: fallbackResponse || 'Sorry, I couldn’t find an answer. Try rephrasing your question!' })
         } catch (fallbackError) {
           res.status(500).json({ error: 'Both APIs failed. Please try again later.' })
         }
       }
     }